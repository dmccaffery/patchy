...
func (r *FindingReconciler) project(ctx context.Context, fnd *v1alpha1.Finding) error {
	...
	return r.projectPhase(ctx, fnd, tracker, repo, number)
}
...
func (r *FindingReconciler) createIssue(
	...
	if err := r.Status().Update(ctx, fnd); err != nil {
		// The issue exists but the link write lost; without the link a
		// re-reconcile would duplicate the issue — surface the error so the
		// retry happens with backoff on this same path.
